...
filtered["cat_resale_price"] = pd.qcut(filtered["resale_price"], [0,0.4,0.6,1], labels=labels)
cat_resale_price = pd.DataFrame(pd.qcut(filtered["resale_price"], [0,0.4,0.6,1]).value_counts().reset_index())
cat_resale_price["index"] = cat_resale_price["index"].astype(str)
cat_resale_price = cat_resale_price.sort_values("index", ascending=True).reset_index().drop(["level_0", "resale_price"], axis=1)
cat_resale_price = cat_resale_price.rename(columns={"index": "Resale Price"})
cat_resale_price["Resale Price"] = cat_resale_price["Resale Price"].apply(lambda x: x.strip('()[]'))
cat_resale_price["Resale Price"] = cat_resale_price["Resale Price"].apply(lambda x: x.replace(",", " to"))
...
select_lease = st.selectbox("Select remaining lease years", sorted(list(filtered["cat_remaining_lease_years"].unique())))
...
for lat, lon, address, town, price, lease, level, cat_resale_price in zip(filtered_sub["latitude"], filtered_sub["longitude"], filtered_sub["address"], filtered_sub["town"], filtered_sub["resale_price"], filtered_sub["remaining_lease_years"], filtered_sub["storey_range"], filtered_sub["cat_resale_price"]):
